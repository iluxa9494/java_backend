...
# --- ТОЛЬКО ЭТА ПАПКА ---
ROOT_DIR="/Users/ilia/IdeaProjects/pet_projects/java_backend/hotel-booking"

# куда пишем результаты (рядом со скриптом)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

OUT_DUMP="${SCRIPT_DIR}/hotel-booking_dump.txt"
OUT_EXCL="${SCRIPT_DIR}/hotel-booking_dump_excluded.txt"

# временный output (чтобы не самозациклиться)
TMP_DUMP="${SCRIPT_DIR}/.hotel-booking_dump.tmp"

# принудительно включаемые файлы (если существуют) — даже если обычно были бы исключены
FORCE_INCLUDE=(
  "${ROOT_DIR}/.gitignore"
  "${ROOT_DIR}/Dockerfile"
...
while IFS= read -r -d '' f; do
  ...
  # FORCE_INCLUDE — только если файл реально существует (иначе он и так не попадёт в find)
  if is_force_include "$f"; then
    write_included "$rel" "$f"
    continue
  fi
