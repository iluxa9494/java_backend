...
public class RoomSpecification {
    ...
    public static Specification<Room> byGuests(Integer minGuests, Integer maxGuests) {
        return (root, query, cb) -> {
            ...
            if (minGuests == null) return cb.lessThanOrEqualTo(root.get("guests"), maxGuests);
            if (maxGuests == null) return cb.greaterThanOrEqualTo(root.get("guests"), minGuests);
            return cb.between(root.get("guests"), minGuests, maxGuests);
        };
    }
